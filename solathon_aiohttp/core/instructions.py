...
def create_account(
    ...
    account_metas: list[AccountMeta] = [
        AccountMeta(
            public_key=from_public_key, is_signer=True, is_writable=True
        ),
        AccountMeta(
            public_key=new_account_public_key, is_signer=True, is_writable=True
        ),
    ]
    data: bytes = SYSTEM_INSTRUCTIONS_LAYOUT.build(
        dict(
            ...
            args=dict(
                lamports=lamports, space=space, program_id=bytes(program_id)
            ),
        )
    )
    return Instruction(
...
def create_account_with_seed(
    ...
    if base_public_key != from_public_key:
        account_metas.append(
            AccountMeta(
                public_key=base_public_key, is_signer=True, is_writable=False
            )
        )
    return Instruction(
...
def assign(
    account_public_key: PublicKey, program_id: PublicKey
) -> Instruction:
    ...
    return Instruction(
        keys=[
            AccountMeta(
                public_key=account_public_key, is_signer=True, is_writable=True
            ),
        ],
        program_id=SYSTEM_PROGRAM_ID,
        data=data,
...
def transfer(
    ...
    account_metas: list[AccountMeta] = [
        ...
        AccountMeta(
            public_key=to_public_key, is_signer=False, is_writable=True
        ),
    ]
    data: bytes = SYSTEM_INSTRUCTIONS_LAYOUT.build(
        dict(type=InstructionType.TRANSFER, args=dict(lamports=lamports))
    )
    return Instruction(
        keys=account_metas,
        program_id=SYSTEM_PROGRAM_ID,
