...
class Transaction:
    def __init__(self, **config):
        ...
        if "instructions" in config:
            ...
            if type(instructions) is list and isinstance(
    ...
    def compile_transaction(self) -> bytes:
        ...
        for instruction in self.instructions:
            if not instruction.program_id:
                raise AttributeError(
                    "Invalid instruction (no program ID found): ", instruction
                )
        ...
        for a_m in account_metas:
            ...
            if public_key in seen:
                idx = seen[public_key]
                uniq_metas[idx].is_writable = (
                    uniq_metas[idx].is_writable or a_m.is_writable
                )
            else:
        ...
        if not self.signatures:
            self.signatures = [
                PKSigPair(public_key=PublicKey(key), signature=None)
        ...
        compiled_instructions: list[CompiledInstruction] = [
            CompiledInstruction(
                accounts=[
                    account_indices[str(a_m.public_key)] for a_m in instr.keys
                ],
    ...
    def sign(self) -> None:
        ...
        pk_sig_pairs: list[PKSigPair] = [
            PKSigPair(public_key=to_public_key(signer))
    ...
    def serialize(self) -> bytes:
        ...
        if len(wire_transaction) > PACKET_DATA_SIZE:
            raise RuntimeError(
                "Transaction too large: ", len(wire_transaction)
            )
    ...
    @classmethod
    def populate(
        self, message: Message, signatures: List[bytes]
    ) -> Transaction:
        decoded_signatures = list(
            map(
                lambda x: PKSigPair(
                    public_key=message.account_keys[x[0]],
                    signature=None
                    if x[1] == b58encode(DEFAULT_SIGNATURE)
                    else b58decode(x[1]),
                ),
        ...
        for instruction in message.instructions:
            ...
            for account in instruction.accounts:
                ...
                acc_metas.append(
                    AccountMeta(
                        public_key=pubkey,
                        is_signer=message.is_account_signer(account)
                        or pubkey
                        in [x.public_key for x in decoded_signatures],
    ...
    def from_buffer(self, buffer: bytes) -> Transaction:
        # Reference:
        # https://github.com/solana-labs/solana-web3.js/blob/a1fafee/packages/library-legacy/src/transaction/legacy.ts#L878
