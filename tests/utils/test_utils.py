import pytest
from solathon_aiohttp import utils


def get_rounded_expectation(arg):
    return utils.truncate_float(
        arg * utils.SOL_PER_LAMPORT, utils.SOL_FLOATING_PRECISION
    )
...
sol_to_lamport_test_data = [
    ...
    (
        ...
        int(123123123123.123123123123 * utils.LAMPORT_PER_SOL),
    ),
]
...
@pytest.mark.parametrize(
    "arg, expected",
    [(-arg, -expected) for arg, expected in sol_to_lamport_test_data],
)
...
@pytest.mark.parametrize(
    "arg, expected",
    [(-arg, -expected) for arg, expected in lamport_to_sol_test_data],
)
